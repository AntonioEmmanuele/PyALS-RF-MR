...
class NEW_ResiliencyBasedGREP2(NEW_GREP):
    ...
    def __init__(self, classifier : Classifier, pruning_set_fraction : float = 0.5, max_loss : float = 5.0, min_resiliency : int = 0, ncpus : int = cpu_count(), validation_epoch : float = 0.1, prune_method: str = "max"):
        self.global_impact_evaluation = {
            "max":  NEW_ResiliencyBasedGREP2.can_prune_max,
            "mean": NEW_ResiliencyBasedGREP2.can_prune_mean,
            "min":  NEW_ResiliencyBasedGREP2.can_prune_min,
        }
    ...
    """ Algorithm:  
        ...
        while len(R) > 0: 
            ...
            for leaf in leaves:
                ...
                if leaf not in pruning_cfg and can_prune_global_impact(residual_redundancy):
                    pruning_cfg.append(leaf)
    ...
    def trim(self, cost_criterion : NEW_GREP.CostCriterion):
        ...
        while len(processing_list) > 0:
            ...
            if actual_redundancy >= self.min_resiliency:        
                ...
                for tree_name, class_name, leaf in tqdm(active_leaves, total = len(active_leaves), desc="Evaluating leaves...", bar_format="{desc:30} {percentage:3.0f}% |{bar:40}{r_bar}{bar:-10b}", leave=False):
                    ...
                    if leaf_id not in self.pruning_configuration and self.can_prune_global_impact(residual_redundancy):
        ...
        #self.val_accuracy = self.evaluate_accuracy(self.args_evaluate_validation, self.y_val)[0]
        self.val_acc_outcomes = self.visit(self.args_evaluate_validation)
        self.val_accuracy = self.acc_eval(self.val_acc_outcomes, self.y_val)
        self.val_per_class_acc = self.eval_per_class_acc(self.val_acc_outcomes, self.y_val)
        self.val_per_class_loss = self.compute_per_class_loss(self.baseline_per_class_acc_validation, self.val_per_class_acc)

        self.loss_val = self.baseline_acc_validation - self.val_accuracy
        ...
        self.test_acc_outcomes = self.visit(self.args_evaluate_test)
        self.test_accuracy = self.acc_eval(self.test_acc_outcomes, self.y_test)
        self.loss_test = self.baseline_accuracy_test - self.test_accuracy
        self.test_per_class_acc = self.eval_per_class_acc(self.test_acc_outcomes, self.y_test)
        self.test_per_class_loss = self.compute_per_class_loss(self.baseline_per_class_acc_test, self.test_per_class_acc)
