        ...
        for rk in ${ranking_procedure[@]}; do
            for name in ${input_datasets[@]}; do # For each input dataset, i.e. avila, spambase 
                ...
                if [[ "$fraction" -eq "0" ]]; then
                    ...
                    $tool mr_heu -c $config_director -p $pruning_directory -d $report_directory -m $mr_order -r $rk -q "int16" -j 20
                else
                    $tool mr_heu -c $config_director -p $pruning_directory -d $report_directory -f $fraction -m $mr_order -r $rk -q "int16" -j 20
                fi
            done
        done
    done
